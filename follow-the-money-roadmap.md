...
- **Netlify is the runtime, not the safe.** Secret env vars (lock icon) are **write-only** — once saved you can't view them again, only overwrite. So every secret needs a **second copy in a real password manager** (Apple **Passwords app** / iCloud Keychain, which follows your Apple ID and syncs — *not* the local Keychain Access login keychain, which is Mac-local and dies with the machine; or 1Password/Bitwarden). Same value in both places: Netlify = working copy, vault = backup.
