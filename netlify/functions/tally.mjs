...
// never a user, a session, an IP, or a cookie. One blob PER PUZZLE, keyed by
// the puzzle id, so pings for different puzzles never contend:
//   key "<puzzle-id>" -> { starts, complete, perfect, cleared, finished, timeSum, timeCount }
...
export default async (req) => {
  ...
  if (!puzzle || (d.event !== "start" && d.event !== "complete")) return new Response(null, { status: 400 });

  try {
    const store = getStore("puzzle-stats");
    const s = (await store.get(puzzle, { type: "json" })) ||
      { starts: 0, complete: 0, perfect: 0, cleared: 0, finished: 0, timeSum: 0, timeCount: 0 };
